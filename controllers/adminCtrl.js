...
const adminCtrl = {
  ...
  getTotalLikelefts: async (req, res) => {
    try {
      const posts = await Posts.find();
      let total_likelefts = 0;
      await posts.map((post) => (total_likelefts += post.likelefts.length));
      res.json({ total_likelefts });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },
  getTotalLikerights: async (req, res) => {
    try {
      const posts = await Posts.find();
      let total_likerights = 0;
      await posts.map((post) => (total_likerights += post.likerights.length));
      res.json({ total_likerights });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },
