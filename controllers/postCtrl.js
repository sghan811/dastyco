...
const postCtrl = {
  createPost: async (req, res) => {
    try {
      const { content, contentsub, images } = req.body;
  ...
  likeleftPost: async (req, res) => {
    try {
      ...
      if (post.length > 0) {
        return res
          .status(400)
          .json({ msg: "You have already likeleftd this post" });
      }

      const likeleft = await Posts.findOneAndUpdate(
        { _id: req.params.id },
        {
      ...
      res.json({ msg: "Post likeleftd successfully." });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },

  unLikeleftPost: async (req, res) => {
    try {
      const likeleft = await Posts.findOneAndUpdate(
        { _id: req.params.id },
        {
      ...
      res.json({ msg: "Post unlikeleftd successfully." });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },

  likerightPost: async (req, res) => {
    try {
      const post = await Posts.find({
        _id: req.params.id,
      ...
      if (post.length > 0) {
        return res
          .status(400)
          .json({ msg: "You have already likerightd this post" });
      }

      const likeright = await Posts.findOneAndUpdate(
        { _id: req.params.id },
        {
      ...
      res.json({ msg: "Post likerightd successfully." });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },

  unLikerightPost: async (req, res) => {
    try {
      const likeright = await Posts.findOneAndUpdate(
        { _id: req.params.id },
        {
      ...
      res.json({ msg: "Post unlikerightd successfully." });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },
